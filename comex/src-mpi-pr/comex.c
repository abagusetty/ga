...
#define ENABLE_PROFILE
  ...
  static double t_nb_putv = 0.0;
  static double t_nb_getv = 0.0;
  static double t_nb_accv = 0.0;
...
#endif
#ifdef ENABLE_DEVICE
STATIC reg_entry_t* _comex_malloc_local_memdev(size_t size, int device_id);
...
#ifdef ENABLE_DEVICE
int comex_malloc_dev(void *ptrs[], size_t size, comex_group_t group)
{
    comex_igroup_t *igroup = NULL;
    reg_entry_t *reg_entries = NULL;
    reg_entry_t my_reg;
...
int comex_free_dev(void *ptr, comex_group_t group)
{
    ...
    if (ptr == NULL) {
      ...
      reg_entry_t *my_reg = reg_cache_find(my_world_rank,ptr, 0, _comex_dev_id);
...
STATIC void _put_iov_handler(header_t *header, int proc)
{
    ...
    packed_index = 0;
    reg_entry = reg_cache_find(header->rank, dst[0], bytes, -1);
#ifdef ENABLE_DEVICE
    if (!reg_entry) {
      reg_entry = reg_cache_find(header->rank, dst[0], bytes,
          _device_map[header->rank]);
    }
#endif
    COMEX_ASSERT(reg_entry);
...
STATIC void _get_iov_handler(header_t *header, int proc)
{
    ...
    packed_index = 0;
    reg_entry = reg_cache_find(
        header->rank, src[0], bytes, -1);
#ifdef ENABLE_DEVICE
    if (!reg_entry) {
      reg_entry = reg_cache_find(
          header->rank, src[0], bytes, _device_map[header->rank]);
    }
#endif
    COMEX_ASSERT(reg_entry);
    ...
    if (use_eager) {
...
#else
      {
        ...
        for(i=0; i<n1dim; i++) {
          ...
          _acc_dev(acc_type, count[0], dst+dst_idx,
              (char*)dev_buffer+packed_index, scale);
          packed_index += count[0];
        }
...
STATIC void _acc_iov_handler(header_t *header, char *scale, int proc)
{
    ...
    packed_index = 0;
    reg_entry = reg_cache_find(
        header->rank, dst[0], bytes, -1);
#ifdef ENABLE_DEVICE
    if (!reg_entry) {
      reg_entry = reg_cache_find(
          header->rank, dst[0], bytes, _device_map[header->rank]);
    }
#endif
    COMEX_ASSERT(reg_entry);
    mapped_offset = _get_offset_memory(reg_entry, dst[0]);
      ...
      if (proc == g_state.rank) {
        ...
        for(i=0; i<n1dim; i++) {
          ...
          if (on_host) {
            ...
            copyPeerToPeer((char*)mapped_offset+dst_idx, _device_map[proc],
            ...
            if (fence_array[g_state.master[proc]]) {
              ...
              if (reg_entry->use_dev && on_host) {
                ...
                for (i=0; i<iov_len; ++i) {
                  ...
                  limit = iov[i].count;
                  for (j=0; j<limit; j++) {
                    _acc(datatype, bytes, dst[j], src[j], scale);
                  }
                }
#ifdef ENABLE_DEVICE
              }
